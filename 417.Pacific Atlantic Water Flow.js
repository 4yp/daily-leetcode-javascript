...
var pacificAtlantic = function(heights) {
  const m = heights.length, n = heights[0].length, dirs = [[1, 0], [-1, 0], [0, 1], [0, -1]], res = []
  const pacific = new Array(m).fill(0).map(() => new Array(n).fill(false)), 
        atlantic = new Array(m).fill(0).map(() => new Array(n).fill(false))
  const dfs = (row, col, visited) => {
    if (visited[row][col]) return
    visited[row][col] = true
    if (pacific[row][col] && atlantic[row][col]) res.push([row, col])
    for ([r, c] of dirs) {
      const nr = row + r, nc = col + c
      if (nr < 0 || nr >= m || nc < 0 || nc >= n || heights[nr][nc] < heights[row][col]) continue
      dfs(nr, nc, visited)
    }
  }
  for (let i = 0; i < m; i++) {
